...
class Client(object):
    ...
    def load_train_data(self, batch_size=None):
        ...
        #测试数据为空时，用训练数据替代
        if len(test_data) == 0:
            test_data = read_client_data(self.dataset, self.id, is_train=True, few_shot=self.few_shot)
            print(f"Warning: Client {self.id} test data is empty, use train data instead")
    ...
    def load_test_data(self, batch_size=None):
        ...
        # 2. 若数据为空，强制设batch_size=1并生成1条规范虚拟数据
        if len(test_data) == 0:
            import torch
            dummy_img = torch.randn(3, 320, 320).type(torch.float32)  # CHW格式！
            dummy_label = torch.tensor(0).type(torch.int64)            
            test_data = [(dummy_img, dummy_label)]
            batch_size = 1  
            print(f"Client {self.id} test data empty, use 1 dummy sample")
